...
- **样式**: Tailwind CSS v4
- **PDF处理**: PDF.js (pdfjs-dist ^3.11)
- **图标**: Lucide React
- **缓存**: IndexedDB (优先) + localStorage 降级
- **部署**: Vercel (香港 hkg1 区域)
...
### PDF文件优化
- 原始 471MB PDF 分割为 39 个功能章节
- 每章节控制在 50MB 以内，大章节（33、59、61、75、79）分为 part1/part2
- 章节按文件大小升序加载（先小后大），提升初始可用时间
- 覆盖 1368 页内容（原始 PDF 1504 页的第 39–1406 页）
